...
class Cont_Cla_Dataset(data.Dataset):
    def initialize(self, files, dataset_sizes,
    ...
    def __getitem__(self, index):
        ...
        source_tensor = transform_image(img)
...
def create_imagemasklabel_dataloader(imgroot, maskroot, files, dataset_sizes, labels, batchSize=16, shuffle=True,
                                     nworkers=4,
                                     preprocess_mode='resize_and_crop', load_size=286, crop_size=256, pair=False):
    imagemasklabeldataset = ImageMaskLabelDataset()
    imagemasklabeldataset.initialize(imgroot, maskroot, files, dataset_sizes, labels, preprocess_mode, load_size,
                                     crop_size, pair)
